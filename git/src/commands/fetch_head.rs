...
impl fmt::Display for FetchHeadEntry {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        writeln!(
            ...
            self.commit_hash, self.label, self.branch_name, self.url_remote
        )
    }
}
...
impl FetchHead {
    ...
    ///
    pub fn new(references: &Vec<Reference>, url_remote: &str) -> Result<FetchHead, CommandsError> {
        let mut entries = HashMap::new();
        for reference in references {
            let commit_hash = reference.get_hash().to_string();
            let branch_name = &reference.get_name().to_string();
            let entry = FetchHeadEntry::new(
                commit_hash,
...
mod tests {
    ...
    fn test_fetch_head_entry_new_from_line_not_for_merge() {
        ...
        assert_eq!(
            entry.commit_hash,
            "d3214e19f4736504392664d579ce1ef2d15b5581"
        );
    ...
    fn test_fetch_head_entry_new_from_line_merge() {
        let line =
            "d3214e19f4736504392664d579ce1ef2d15b5581		branch 'main' of github.com:example/repo";
        let result = FetchHeadEntry::new_from_line(line);
        assert!(result.is_ok());
    ...
    fn test_new_fetch_head_with_references() {
        ...
        let references = vec![
            Reference::new(
                "93455fe53543e1dcca9533dd51d5b83656a6432c",
                "refs/heads/branch1",
            )
            .unwrap(),
            Reference::new(
                "56620fe39508e1dcca4873dd51d5b83656a9418c",
                "refs/heads/branch2",
            )
            .unwrap(),
        ];
    ...
    pub fn test_update_references() {
        let references = vec![
            ...
            .unwrap(),
            Reference::new(
                "98094u50hj3094u503498hj0439h804308434380",
                "refs/heads/branch3",
            )
            .unwrap(),
        ];
        ...
        let references = vec![
            ...
            .unwrap(),
            Reference::new(
                "92834092834093u24098u2098u034jhr984h9843",
                "refs/heads/branch2",
            )
            .unwrap(),
        ];
    ...
    pub fn test_fetch_head_write() {
        let references = vec![
            ...
            .unwrap(),
            Reference::new(
                "09h90238h9r298h2832r98h23r98h32r98h23293",
                "refs/heads/branch3",
            )
            .unwrap(),
            Reference::new(
                "1n3291u7b9192h812921397891h98132hg971133",
                "refs/heads/branch4",
            )
            .unwrap(),
        ];
