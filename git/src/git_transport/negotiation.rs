...
pub fn receive_request(stream: &mut dyn Read) -> Result<Vec<String>, UtilError> {
    ...
    // Want
    let want = receive_request_type(stream, "want")?;
    request.extend(want);
...
fn receive_request_type(stream: &mut dyn Read, type_req: &str) -> Result<Vec<String>, UtilError>
{
...
mod tests {
    ...
    fn test_receive_request_type_valid_have() {
        ...
        let result = receive_request_type(&mut stream, "have");
        assert!(result.is_ok());
