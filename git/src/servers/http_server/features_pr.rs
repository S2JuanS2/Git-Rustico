...
use super::pr_registry::{
    delete_pr_map, generate_head_base_hash, generate_pr_hash_key, pr_already_exists, read_pr_map,
    update_pr_map,
};
use super::utils::{
    get_next_pr_number, save_pr_to_file, setup_pr_directory, valid_repository,
...
pub fn create_pull_requests(
    ...
    if validate_existing_pr(body, &path) {
        return Ok(StatusCode::ValidationFailed(
            "The pull request already exists.".to_string(),
        ));
    }
...
pub fn get_pull_request(
    ...
    if !file_exists(&file_path) {
        return Ok(StatusCode::ResourceNotFound(
            "The pull request does not exist.".to_string(),
        ));
    }
    ...
    Ok(StatusCode::Ok(Some(Model::ListPullRequest(Vec::from([
        pr,
    ])))))
}
...
pub fn merge_pull_request(
    ...
    if body.get_field("state")? != OPEN {
        return Ok(StatusCode::InternalError(
            "This pull request is closed".to_string(),
        ));
    }
    let directory = format!("{}/{}", src, repo_name);
    ...
    let updated_body = match serde_json::to_string(&pr) {
        ...
        Err(_) => {
            return Ok(StatusCode::InternalError(
                "Serialize error JSON".to_string(),
            ));
        }
    };
    ...
    if let Err(e) = delete_pr_in_map(
        &updated_body_http,
        &format!("{}/{}/{}", src, PR_FOLDER, repo_name),
    ) {
        return Ok(e);
    };
...
fn extract_pr_fields(body: &HttpBody) -> Result<(String, String, String, String), StatusCode> {
    let head = body
        .get_field("head")
        .map_err(|_| StatusCode::InternalError("Missing head field".to_string()))?;
    let base = body
        .get_field("base")
        .map_err(|_| StatusCode::InternalError("Missing base field".to_string()))?;
    let owner = body
        .get_field("owner")
        .map_err(|_| StatusCode::InternalError("Missing owner field".to_string()))?;
    let title = body
        .get_field("title")
        .map_err(|_| StatusCode::InternalError("Missing title field".to_string()))?;
...
fn update_pr_attributes(
    ...
    match pull_number.parse::<usize>() {
        Ok(value) => add_attributes(directory, body, pr, value)
            .map_err(|_| StatusCode::InternalError("Failed to add attributes".to_string())),
        Err(_) => Err(StatusCode::InternalError(
            "Invalid pull request number".to_string(),
        )),
    }
}

pub fn modify_pull_request(
    body: &HttpBody,
    repo_name: &str,
    pull_number: &str,
    src: &String,
    ...
    let n_pull_number = match pull_number.parse::<usize>() {
        Ok(value) => value,
        Err(_) => {
            return Ok(StatusCode::BadRequest(
                "Invalid pull request number".to_string(),
            ))
        }
    };
...
pub fn change_base_in_pr(
    ...
    if !pr.is_open() {
        return Err(StatusCode::ValidationFailed(
            "El pr esta cerrado. No se acepta modificaciones".to_string(),
        ));
    }
    ...
    if !validate {
        return Err(StatusCode::ValidationFailed(
            "No changes between branches".to_string(),
        ));
    }
    let pr_repo_folder_path = format!("{}/{}/{}", src, PR_FOLDER, repo_name);
    let pr_map_path = format!("{}/{}", pr_repo_folder_path, PR_MAP_FILE);
    ...
    let old_base = match pr.get_base() {
        ...
        None => {
            return Err(StatusCode::InternalError(
                "No base branch en el pr".to_string(),
            ))
        }
    };
...
fn add_attributes(
    ...
    let mergeable = is_mergeable(
        directory,
        &body.get_field("base")?,
        &body.get_field("head")?,
    )?;
    pr.change_mergeable(&mergeable.to_string());
    let changed_files = get_changed_files_pr(
        directory,
        &body.get_field("base")?,
        &body.get_field("head")?,
    )?;
    pr.set_changed_files(changed_files);
    let commits = get_commits_pr(
        directory,
        &body.get_field("base")?,
        &body.get_field("head")?,
    )?;
...
///
pub fn read_and_validate_pull_request(
    repo_name: &str,
    pull_number: &str,
    src: &String,
) -> Result<PullRequest, StatusCode> {
    if valid_repository(repo_name, src).is_err() {
    ...
    let pr = match PullRequest::create_from_file(&file_path) {
        ...
        Err(_) => {
            return Err(StatusCode::InternalError(
                "Error al leer la solicitud de extracción en la base de datos".to_string(),
            ))
        }
    };
...
fn get_pull_request_file_path(repo_name: &str, pull_number: &str, src: &String) -> String {
    format!(
        "{}/{}/{}/{}{}",
        src, PR_FOLDER, repo_name, pull_number, PR_FILE_EXTENSION
    )
}
...
///
fn get_body_commits_pr(
    body: HttpBody,
    src: &str,
    repo_name: &str,
) -> Result<Vec<CommitsPr>, ServerError> {
    let head = body.get_field("head")?;
        ...
        for line in lines_commit.by_ref() {
            ...
            if parts.len() >= 4 {
                if line.starts_with("author") {
                    ...
                    commits_pr.date = chrono::DateTime::from_timestamp(timestamp, 0)
                        .unwrap()
                        .to_string();
...
fn recovery_tree_pr(
    directory: &str,
    pr_files_map: &mut HashMap<String, String>,
    tree_hash_head: &str,
    path: &str,
) -> Result<(), ServerError> {
...
///
pub fn check_pull_request_changes(
    repo_name: &str,
    src: &str,
    body: &HttpBody,
) -> Result<(), StatusCode> {
    match PullRequest::check_pull_request_validity(repo_name, src, body) {
        Ok(changes) => {
            if !changes {
                return Err(StatusCode::ValidationFailed(
                    "The pull request does not contain any changes.".to_string(),
                ));
            }
        }
        Err(e) => return Err(StatusCode::InternalError(e.to_string())),
